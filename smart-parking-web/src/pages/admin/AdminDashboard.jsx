...
import { Home, Activity, Bell, MonitorPlay, CreditCard, MapPin, Receipt, Menu, Search, Settings, ChevronDown, CheckCircle, XCircle, AlertTriangle, Car } from 'lucide-react';
import { useNavigate, useLocation } from 'react-router-dom';

// ─────────────────────────────────────────────────────────────
// SIDEBAR ITEM COMPONENT
// ─────────────────────────────────────────────────────────────
const SidebarItem = ({ icon: Icon, label, isActive, isCollapsed }) => {
    return (
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'px-6'} py-4 cursor-pointer transition-colors duration-200 
...
export default function AdminDashboard() {
    ...
    const user = state?.user || { name: 'Nguyen Van A', role: 'System Administrator' };
    
    const [isCollapsed, setIsCollapsed] = useState(false);

    // Menu Configuration
    const menuItems = [
        { icon: Home, label: 'Smart Home', isActive: false },
        { icon: Activity, label: 'Activity report', isActive: true }, // Default active
        { icon: Bell, label: 'Alerts', isActive: false },
        { icon: MonitorPlay, label: 'Devices', isActive: false },
        { icon: CreditCard, label: 'Credentials cards', isActive: false },
        { icon: MapPin, label: 'Parking places', isActive: false },
        { icon: Receipt, label: 'Fee policy', isActive: false },
    ];
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto py-4">
                    {menuItems.map((item, idx) => (
                        <SidebarItem key={idx} icon={item.icon} label={item.label} isActive={item.isActive} isCollapsed={isCollapsed} />
                    ))}
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                <header className="h-[70px] bg-white border-b border-gray-200 px-6 flex items-center justify-between shrink-0">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <button className="text-gray-500 hover:text-[#2d3a8c] transition-colors">
                            <Settings size={22} />
                        </button>
                        <div className="h-8 w-[1px] bg-gray-200"></div>
                        <div className="flex items-center gap-3 cursor-pointer">
                            <div className="w-[36px] h-[36px] bg-[#2d3a8c] text-white rounded-full flex items-center justify-center font-bold">
                                {user.name.charAt(0)}
                            </div>

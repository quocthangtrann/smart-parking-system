...
// FIX #3: Use slot.state (not slot.status) and compare lowercase enum values
// ('empty' | 'occupied' | 'active' | 'reserved' | 'maintenance' | 'error')
  ...
  return (
    <div className="grid grid-cols-5 gap-4 max-w-[500px] mx-auto p-4">
      {slots.map((slot) => {
        ...
        // Normalize: use slot.state, handle both 'occupied' and 'active' as taken
        const state = (slot.state || '').toLowerCase();
        const isReserved  = state === 'reserved';
        const isOccupied  = state === 'occupied' || state === 'active';
        const isMaintenance = state === 'maintenance' || state === 'error';
        ...
        return (
          <button
            key={slot.id}
            disabled={isOccupied || isReserved || isMaintenance || (isAdmin && isAvailable)}
            onClick={() => onSlotClick(slot.id)}
            className={`
              relative aspect-[3/4] rounded-xl border-2 flex flex-col items-center justify-center transition-all duration-200
              ${borderColor} ${bgColor} ${textColor}
              ${(isOccupied || isReserved || isMaintenance || isAdmin) ? 'cursor-not-allowed opacity-80' : 'hover:shadow-lg hover:scale-105 cursor-pointer'}
          ...
          >
            ...
            {isMaintenance && <span className="text-[9px] font-bold mt-1">MNT</span>}
          </button>
        );
      })}
    </div>
